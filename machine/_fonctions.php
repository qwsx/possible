...
include('_bdd.php');
include('_langue.php');
require_once('_lib/_recaptchalib.php');
...
function tagPossible($idDec,$lan){
	...
	while($result = mysql_fetch_array($sql1)){
		...
		$ret["hashTag"] .= "#".ucwords($result['tag'])." ";
	}
	return $ret;
}
//....................................................................
function possibleTag($tag){
	...
	while($result = mysql_fetch_array($sql1)){
		if(!in_array($result['idDec'],$idSrc)){
			$idSrc[] = $result['idDec'];
			$ret['img'] .= '<a href="'.$result['idDec'].'.'.$GLOBALS['lan'].'"><img src="'.mkId(decbin($result['idDec'])).'-9-0.png" alt="'.$result['tag'].'" title="'.$result['tag'].'"/></a>';
		}
...
function  dernierDecouverte(){
	$sql = mysql_query(" SELECT * FROM `possible` ORDER BY `possible`.`id` DESC LIMIT 0 , 15 ");
	$lan = $GLOBALS['lan'];
	while($result = mysql_fetch_array($sql)){
		$tagPossible = tagPossible($result['idDec'],$lan);
		$titre = antiHack($tagPossible["0v"].$tagPossible["1v"]);
		$res .= '<a href="'.$result['idDec'].'.'.$lan.'"><img title="'.$titre.'" src="'.mkId(decbin($result['idDec'])).'-9-0.png" ></a>';
	}
	return $res;
}
//....................................................................
...
function pixSiteMap ($langue) {
	...
	while ($ligne = mysql_fetch_array($requete)){
		...
		$prio = 0.7;
		$sitePortail = "http://possible.dcfvg.com";
		$siteMap .= '
				<url>
...
function GenSiteMap($langue) {
	...
	$siteMap.='
			<url>
				<loc>http://possible.dcfvg.com/</loc>
				<lastmod>'.$mod.'</lastmod>
				<priority>0.9</priority>
			</url>
			<url>
				<loc>http://possible.dcfvg.com/words.fr</loc>
				<lastmod>'.$mod.'</lastmod>
				<priority>0.9</priority>
			</url>
			<url>
				<loc>http://possible.dcfvg.com/seeAlso.fr</loc>
				<lastmod>'.$mod.'</lastmod>
				<priority>0.9</priority>
			</url>
			<url>
				<loc>http://possible.dcfvg.com/*.fr</loc>
				<lastmod>'.$mod.'</lastmod>
				<priority>0.9</priority>
			</url>
			<url>
				<loc>http://possible.dcfvg.com/rand.fr</loc>
				<lastmod>'.$mod.'</lastmod>
				<priority>0.9</priority>
			</url>
		</urlset>';
